...
export async function POST(request:Request){
 ...
 try{
  ...
  if(testOrderId){try{updateOrderStatus(testOrderId,'cancelled','received','system:yoco-sandbox');}catch{/* Preserve the original provider error. */}}
  return Response.json({message:error instanceof Error?error.message:'Could not create Yoco sandbox checkout.'},{status:400});
 }
}
