...
export async function GET(){if(!await actor())return Response.json({message:'Super admin access required.'},{status:403,headers:noStore});const base=publicBaseUrl();return Response.json({configured:Object.fromEntries(names.map(n=>[n,providerSecretStatus(n)])),yocoMode:yocoCredentialMode(),webhookUrl:base?`${base}/api/payments/webhook`:'Public URL not configured',whatsappWebhookUrl:base?`${base}/api/webhooks/whatsapp`:'Public URL not configured',vaultReady:/^[a-f0-9]{64}$/i.test(process.env.FOND_CREDENTIALS_KEY??''),emailFrom:emailSender(),twilio:twilioConfig(),meta:metaConfig()},{headers:noStore});}
