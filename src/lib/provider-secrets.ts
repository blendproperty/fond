...
export type ProviderName='yoco-secret'|'yoco-webhook'|'email-api'|'staff-shared-code'|'twilio-auth-token';
...
export function saveProviderSecret(name:ProviderName,value:string,actor:string){
 if(!value||value.length>500||name==='yoco-secret'&&!/^sk_(live|test)_/.test(value)||name==='yoco-webhook'&&!value.startsWith('whsec_')||name==='email-api'&&!value.startsWith('re_')||name==='staff-shared-code'&&!/^[A-Za-z0-9._-]{6,64}$/.test(value)||name==='twilio-auth-token'&&!/^[a-f0-9]{32}$/i.test(value))throw new Error('Enter a valid credential or staff code.');
