...
export function enqueueNotification(orderId:string,status:string){
  ...
  const db=getDb(),o=db.prepare('SELECT whatsapp_opt_in,contact_number FROM orders WHERE id=?').get(orderId);
  if(!o?.whatsapp_opt_in||!o.contact_number)return;
  db.prepare("INSERT OR IGNORE INTO notification_jobs VALUES (?,?,?,'pending',0,NULL,?,?)").run(randomUUID(),orderId,'order_'+status,Date.now(),new Date().toISOString());
}
