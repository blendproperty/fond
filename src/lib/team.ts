...
type Member={id:string;name:string;username:string;role:'manager'|'staff';active:number;password_hash:string};
...
export function loginAllowed(key:string){
  ...
  return (db.prepare('SELECT count FROM login_attempts WHERE key=?').get(digest(key)) as {count:number}).count<=15;
}
...
export function saveMember(input:{id?:string;name:string;username:string;role:string;active:boolean;password?:string},actor:string){
  if(typeof input.name!=='string'||!input.name.trim()||input.name.length>100||typeof input.username!=='string'||!/^[a-zA-Z0-9._@-]{3,100}$/.test(input.username)||!['manager','staff'].includes(input.role)||typeof input.active!=='boolean')throw new Error('Provide name, unique username and valid role.');
  ...
  if(actor===`team:${input.id}`&&(!input.active||input.role!=='manager'))throw new Error('You cannot remove your own manager access.');
