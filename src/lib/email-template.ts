...
function receiptLayout(input: {
  ...
  const ready=input.variant==='ready';
  const statusBackground=ready?BRAND.green:BRAND.aqua;
  const statusTitle=ready?'#ffffff':BRAND.ink;
  const statusDetail=ready?BRAND.aqua:BRAND.ink;
  return `<!doctype html>
...
const statusLabel = (status: OrderStatus,fulfilment?:OrderRecord['fulfillment']) => status==='ready'?(fulfilment==='delivery'?'Ready for delivery':'Ready for collection'):status==='out_for_delivery'?'Out for delivery':({ received: 'Received', accepted: 'Accepted', preparing: 'Preparing', completed: 'Completed', cancelled: 'Cancelled' }[status]);
...
export function buildOrderEmail(order: OrderRecord, event: 'received' | 'accepted' | 'ready' | 'completed'): EmailMessage {
  ...
  const values={customerName:order.customerName,reference:order.displayReference,prepMinutes:order.estimatedPrepMinutes,collectionTime:order.collectionTime,destination,total:money(order.totalCents)};
  ...
  const text = `FOND Midpoint Hub\n\n${title}\n${status}\n\n${intro}\n\nYOUR ORDER DETAILS\nOrder: ${order.displayReference}\nDate: ${localDate(order.createdAt)}\nName: ${order.customerName}\nEmail: ${order.customerEmail ?? 'Not supplied'}\nMobile: ${order.contactNumber ?? 'Not supplied'}\nStatus: ${statusLabel(event,order.fulfillment)}\nFulfilment: ${fulfilment}\n${fulfilment}: ${order.collectionTime}\nDestination: ${destination}\nPayment: ${payment}${order.note ? `\nNotes: ${order.note}` : ''}\n\n${itemText}\n\nSubtotal: ${money(order.totalCents)}\nTotal: ${money(order.totalCents)}\nAmount due: ${money(amountDue)}\n\nTrack your order: ${publicUrl()}/account\n\nFOND · Midpoint Hub`;
  const details = `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">${detailRow('Order', order.displayReference)}${detailRow('Date', localDate(order.createdAt))}${detailRow('Name', order.customerName)}${order.customerEmail ? detailRow('Email', order.customerEmail) : ''}${order.contactNumber ? detailRow('Mobile', order.contactNumber) : ''}${detailRow('Status', statusLabel(event,order.fulfillment))}${detailRow('Fulfilment', fulfilment)}${detailRow(fulfilment, order.collectionTime)}${detailRow('Destination', destination)}${detailRow('Payment', payment)}${order.note ? detailRow('Notes', order.note) : ''}</table>`;
  ...
  const content = `${sectionTitle('Your order details')}${details}<div style="height:22px;line-height:22px">&nbsp;</div>${divider()}<div style="height:22px;line-height:22px">&nbsp;</div>${sectionTitle('FOND Midpoint')}<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">${itemsHtml}</table>${totals}<div style="height:20px;line-height:20px">&nbsp;</div>${divider()}<p style="margin:20px 0 0;color:${BRAND.ink};font-size:11px;line-height:18px">${escapeHtml(intro)} Keep your order number <strong>${escapeHtml(order.displayReference)}</strong> handy if you need help. You can follow the order from your FOND account.</p>${event==='ready'?promotionPanel(templates.email.banner):''}`;
  return { subject, text, html: receiptLayout({ preview: `${title}: ${order.displayReference}.`, title, statusLine: status, content, footer: `${templates.email.footer} Quote ${order.displayReference} if you contact the FOND team about this order.`, variant:event==='ready'?'ready':'standard' }) };
}
