...
const statusLabel = (status: OrderStatus,fulfilment?:OrderRecord['fulfillment']) => status==='ready'?(fulfilment==='delivery'?'Ready for delivery':'Ready for collection'):({ received: 'Received', accepted: 'Accepted', preparing: 'Preparing', completed: 'Completed', cancelled: 'Cancelled' }[status]);
...
export function buildOrderEmail(order: OrderRecord, event: 'received' | 'accepted' | 'ready' | 'completed'): EmailMessage {
  ...
  const content = `${sectionTitle('Your order details')}${details}<div style="height:22px;line-height:22px">&nbsp;</div>${divider()}<div style="height:22px;line-height:22px">&nbsp;</div>${sectionTitle('FOND Midpoint')}<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">${itemsHtml}</table>${totals}<div style="height:20px;line-height:20px">&nbsp;</div>${divider()}<p style="margin:20px 0 0;color:${BRAND.ink};font-size:11px;line-height:18px">${escapeHtml(intro)} Keep your reference <strong>${escapeHtml(order.reference)}</strong> handy if you need help. You can follow the order from your FOND account.</p>${event==='ready'?promotionPanel(templates.email.banner):''}`;
  return { subject, text, html: receiptLayout({ preview: `${title}: ${order.reference}.`, title, statusLine: status, content, footer: `${templates.email.footer} Quote ${order.reference} if you contact the FOND team about this order.`, variant:event==='ready'?'ready':'standard' }) };
}
