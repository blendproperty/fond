...
function promotionPanel() {
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:24px 0 0;background:${BRAND.green};border-radius:9px;overflow:hidden"><tr><td style="padding:26px 24px;text-align:center"><p style="margin:0 0 6px;color:${BRAND.aqua};font-size:10px;line-height:14px;font-weight:800;letter-spacing:1.8px;text-transform:uppercase">FOND · MIDPOINT HUB</p><p style="margin:0 0 7px;color:#ffffff;font-family:Georgia,'Times New Roman',serif;font-size:25px;line-height:31px;font-weight:700">Good food. Everyday.</p><p style="margin:0 auto 18px;max-width:360px;color:#d9e7e3;font-size:12px;line-height:18px">Fresh breakfast, lunch, coffee and more — ready for collection or delivery.</p><table role="presentation" cellspacing="0" cellpadding="0" border="0" align="center"><tr><td bgcolor="${BRAND.aqua}" style="border-radius:20px"><a href="${escapeHtml(publicUrl())}" style="display:inline-block;padding:10px 19px;color:${BRAND.ink};text-decoration:none;font-size:11px;line-height:14px;font-weight:800">Browse the FOND menu</a></td></tr></table></td></tr></table>`;
}
...
export function buildControlledTestEmail(recipient = 'Your email address'): EmailMessage {
  ...
  return { subject, text, html: receiptLayout({ preview: 'FOND transactional email is connected and ready.', title: 'Your FOND email updates are ready', statusLine: 'Transactional email is connected', content, footer: 'Keep this message as confirmation that your FOND email channel is working.' }) };
}
...
const eventCopy: Record<'received' | 'accepted' | 'ready' | 'completed', { title: string; status: (order: OrderRecord) => string; intro: (order: OrderRecord) => string }> = {
  received: { title: 'Order received by FOND Midpoint', status: order => `Estimated preparation time: ${order.estimatedPrepMinutes} min`, intro: order => `Thanks, ${order.customerName}. We have received your order and the team will review it shortly.` },
  accepted: { title: 'Order accepted by FOND Midpoint', status: order => `Estimated preparation time: ${order.estimatedPrepMinutes} min`, intro: order => `Good news, ${order.customerName}. The FOND team has accepted your order.` },
  ready: { title: 'Your FOND order is ready', status: order => order.fulfillment === 'delivery' ? 'Ready for delivery' : 'Ready for collection at Midpoint Hub', intro: order => order.fulfillment === 'delivery' ? `${order.customerName}, your order is ready and will be delivered shortly.` : `${order.customerName}, your order is ready to collect from FOND at Midpoint Hub.` },
  completed: { title: 'Your FOND order is complete', status: () => 'Thank you for ordering from FOND', intro: order => `${order.customerName}, your order has been completed. We hope you enjoy it.` },
};
...
export function buildOrderEmail(order: OrderRecord, event: 'received' | 'accepted' | 'ready' | 'completed'): EmailMessage {
  ...
  const subject = event === 'received' ? `FOND has received ${order.reference}` : event === 'ready' ? `Your FOND order ${order.reference} is ready` : event === 'accepted' ? `FOND accepted ${order.reference}` : `Thank you for your FOND order ${order.reference}`;
  ...
  const content = `${sectionTitle('Your order details')}${details}<div style="height:22px;line-height:22px">&nbsp;</div>${divider()}<div style="height:22px;line-height:22px">&nbsp;</div>${sectionTitle('FOND Midpoint')}<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">${itemsHtml}</table>${totals}<div style="height:20px;line-height:20px">&nbsp;</div>${divider()}<p style="margin:20px 0 0;color:${BRAND.ink};font-size:11px;line-height:18px">${escapeHtml(copy.intro(order))} Keep your reference <strong>${escapeHtml(order.reference)}</strong> handy if you need help. You can follow the order from your FOND account.</p>${promotionPanel()}`;
  return { subject, text, html: receiptLayout({ preview: `${copy.title}: ${order.reference}.`, title: copy.title, statusLine: copy.status(order), content, footer: `Quote ${order.reference} if you contact the FOND team about this order.` }) };
}
