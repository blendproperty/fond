...
function fixture(created:string,complete=true){const o=createOrder({submissionKey:randomUUID(),customerName:'Analytics fixture',contactNumber:'0821234567',collectionTime:'ASAP',source:'staff',lines:[{id:'espresso-single',quantity:2}]});if(complete){recordPosEntry(o.id,'YOCO-TEST','fixture');updateOrderStatus(o.id,'ready');getDb().prepare('INSERT INTO payment_records VALUES (?,?,?,?,?,?,?)').run(randomUUID(),o.id,o.totalCents,'cash',randomUUID(),'fixture',new Date().toISOString());updateOrderStatus(o.id,'completed');}getDb().prepare('UPDATE orders SET created_at=? WHERE id=?').run(created,o.id);return o;}
