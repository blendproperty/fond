...
test('category arrows reveal hidden categories and installation instructions are prominent',async({page},testInfo)=>{
 ...
 await page.getByRole('tab',{name:'All-Day Breakfast',exact:true}).focus();await page.keyboard.press('End');await expect(page.getByRole('tab',{name:'Food Truck',exact:true})).toHaveAttribute('aria-selected','true');
