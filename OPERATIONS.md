...
Create a named super admin in Settings using the existing shared admin code. Sign in as that named account and enroll 2FA; once enabled, the shared admin code stops working. Save the one-use recovery codes outside FOND. Managers run operations. Owners can review change history, roll back the latest operational change, and manage staff or manager accounts. Super admins additionally control provider credentials, provider switches, Owner/Super Admin accounts and the shared tablet code. Staff work orders on the tablet. Rotating the shared staff code invalidates old tablet sessions. Trading hours enforcement remains off by default; collection choices are preferences, not reserved slots.

Menu, trading, website-content and customer-message edits retain before/after versions under Settings → Change history & rollback. Only the latest change to an item is eligible for rollback, preventing an older version from overwriting newer work. Security, provider, customer, payment and retry activity is retained separately without passwords or provider secrets.
